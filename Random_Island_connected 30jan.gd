...
func strip_horizont(m,n,sign_m):
	set_starter(m+sign_m*7,n,[sign_m,0])
...
func strip_vertical(m,n,sign_n):
	set_starter(m,n+sign_n*7,[0,sign_n])
...
func strip_diagonal(m,n,sign_m,sign_n):
	...
	set_starter(m+sign_m*7,n+sign_n*7,[sign_m,sign_n])
...
func _ready():
	if 1 == 1:         #######################################
		...
		for x in range(m-7,m+8):
			for y in range(n-1,n+2):
				check_tile(x,y)
		set_starter(m-7,n,[-1,0])
		set_starter(m+7,n,[1,0])
		for x in range(m-1,m+2):
			for y in range(n-7,n+8):
				check_tile(x,y)
		set_starter(m,n-7,[0,-1])
		set_starter(m,n+7,[0,1])
		...
		while( (map.size() < \
		(map_width*map_height/1.3)) && \
		(map_tile_select.empty() == false) ):
			
			randomize()
			var pos = randi() % map_tile_select.size()
#			print("pos: ",pos," map_tile_select.keys()[pos]: ",map_tile_select.keys()[pos])
...
#			
			...
			(map_tile_select[Vector2(m,n)][1] == 0):
				print("bla ",map_tile_select[Vector2(m,n)][0]," ",map_tile_select[Vector2(m,n)][1])
				if u == 0:
					u = random[(randi()%2)]
...
#					u = -1
#			else:
#				u = map_tile_select[Vector2(m,n)].x

#			if map_tile_select[Vector2(m,n)].y == 0:
#				if randi()%2:
#					v = 1
#				else:
#					v = -1
#			else:
#				v = map_tile_select[Vector2(m,n)].y
